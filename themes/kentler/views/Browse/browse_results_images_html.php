 ...
 */
	...
	$vn_detail_embed = $this->request->getParameter("detailembed", pInteger);	
		...
		$vn_col_span_xs = 6;
		$vb_refine = false;
		if(!$vn_detail_embed && (is_array($va_facets) && sizeof($va_facets))){
			$vb_refine = true;
			$vn_col_span = 3;
			$vn_col_span_sm = 6;
			$vn_col_span_xs = 6;
		...
		<div class='bResultItem' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
			...
			<div class='bResultItemContent'><div class='text-center bResultItemImg'>{$vs_rep_detail_link}</div>
				<div class='bResultItemText'>
					<small>{$vs_label_detail_link}</small>
			...
			print caNavLink($this->request, _t('Next %1', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $vn_start + $vn_hits_per_block, 'key' => $vs_browse_key, 'view' => $vs_current_view, 'detailembed' => $vn_detail_embed));
		}
?>
