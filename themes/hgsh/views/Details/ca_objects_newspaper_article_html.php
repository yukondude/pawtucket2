...
<div class='col-sm-<?php print ($vb_multiple_reps) ? "7" : "8"; ?>'>
	...
	<div class='detailBlueText'>{{{^ca_objects.type_id, }}}{{{^ca_objects.idno}}}</div>
		{{{<ifcount code="ca_collections" restrictToTypes="collection" min="1">
			<div class="row">
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					jQuery("#browseResultsContainer").load("<?php print caNavUrl($this->request, '', 'Search', 'collections', array('search' => 'object_id:^ca_objects.object_id AND type_id: 106'), array('dontURLEncodeParameters' => true)); ?>", function() {
...
<?php
	...
	if(is_array($va_featured_collections) && sizeof($va_featured_collections)){
		...
		if($q_featured_collections->numHits()){
			print "<div class='row'><div class='col-sm-12'><div class='btn btn-default'>"._t("Article Categories")."</div></div></div><!-- end row -->\n";
			$i = 0;
	...
	$va_collections = $t_item->get("ca_collections", array("returnWithStructure" => true, "restrictToTypes" => array("collection"), 'excludeRelationshipTypes' => array('featured', 'history'),"checkAccess" => $va_access_values));
	if(sizeof($va_collections)){
		...
		foreach($va_collections as $va_collection){
			...
			$vs_thumb = $t_rel_collection->get("ca_object_representations.media.iconlarge", array("checkAccess" => $va_access_values, "limit" => 1));
			if($vs_thumb){
				$vs_thumb = caDetailLink($this->request, $vs_thumb, '', 'ca_objects', $vn_id);		
			} else {
				$vs_thumb = caDetailLink($this->request, $vs_default_placeholder, '', 'ca_objects', $vn_id);
			}
			print "<div class='row'><div class='col-sm-4 col-md-4 col-lg-4 detailRelatedThumb'>".$vs_thumb."</div>\n";
			print "<div class='col-sm-8 col-md-8 col-lg-8'>\n";
