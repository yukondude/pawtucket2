...
<div class="row">		
<div class='col-sm-8'>
	<div class="detailTitleSmall">{{{^ca_places.preferred_labels.name}}}</div>
<?php
...
?>
	{{{<ifdef code="ca_places.description"><p>^ca_places.description</p></ifdef>}}}
	{{{<ifdef code="ca_places.additional_info">
			...
			<p id='additional_info' style='display:none;'>^ca_places.additional_info<br/><a href="#" onClick="jQuery('#additional_info').toggle(); jQuery('#additional_info_link').toggle(); return false;" class="detailMoreInfo">Hide <span class="glyphicon glyphicon-arrow-up"></span></a></p>
	</ifdef>}}}
...
<div class='col-sm-4'>
	<div class="detailTitle">{{{^ca_places.preferred_labels.name}}}</div>
<?php
		...
		if(is_array($va_featured_collections) && sizeof($va_featured_collections)){
			...
			if($q_featured_collections->numHits()){
				...
				while($q_featured_collections->nextHit()){
					...
					if(!($vs_media = $q_featured_collections->getWithTemplate("<unit relativeTo='ca_objects' length='1'><unit relativeTo='ca_object_representations' length='1'>^ca_object_representations.media.resultcrop</unit></unit>", array("checkAccess" => $va_access_values)))){
